...
class Login extends CI_Controller
{
	...
	function do_login()
	{
		...
		$a = $this->db->get_where("fdm_va_auths", array("email" => $email, "deleted" => 0, "status" => "Active"))->row();

		if ($a) {


			$chk1 = $this->db->get_where("fdm_va_auths", array("email" => $email, "deleted" => 0, "status" => "Active"))->num_rows();
			if ($chk1 == 1) {

				$u = $this->db->get_where("fdm_va_auths", array("email" => $email, "deleted" => 0, "status" => "Active"))->row();
